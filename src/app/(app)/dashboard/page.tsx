  ...
  return (
    <div className="mx-auto max-w-5xl space-y-12">
      <header><p className="text-sm text-[var(--text-dim)]">somm@cellar:~$ dashboard --taster={profile.name}</p>
        <h1 className="bloom-cyan mt-2 flex items-center gap-3 text-2xl text-[var(--cyan)]"><Barrel /> ── CELLAR DASHBOARD ──</h1></header>
      <section><h2 className="mb-4 flex items-center gap-2 text-[var(--amber)]"><Bottle size={16} color="var(--amber)" /> ── UP NEXT FOR {profile.name.toLocaleUpperCase()} ──</h2>
        {upNext.length ? <div className="mb-5 grid gap-4 md:grid-cols-2">{upNext.map((item) => <article className="border border-[var(--border)] p-5" key={item.id}><h3>{item.wineName}</h3><p className="mt-1 text-xs uppercase text-[var(--amber)]">{item.grape} {item.region} {item.priceBand}</p><p className="mt-2 text-sm text-[var(--text-dim)]">{item.reasoning}</p><RecommendationStatus id={item.id} /></article>)}</div>
      ...
      {householdProfiles.length >= 2 && <section><h2 className="mb-4 flex items-center gap-2 text-[var(--magenta)]"><Sparkle size={16} color="var(--magenta)" /> ── FOR THE TABLE ──</h2>{joint.length ? <div className="mb-5 grid gap-4 md:grid-cols-2">{joint.map((item) => <article className="border border-[var(--border)] p-5" key={item.id}><h3>{item.wineName}</h3><p className="mt-2 text-sm text-[var(--text-dim)]">{item.reasoning}</p><div className="mt-3 flex gap-2">{householdProfiles.map((householdProfile) => <span className={`bloom-${householdProfile.color}`} style={{ color: `var(--${householdProfile.color})` }} key={householdProfile.id}>{householdProfile.name}</span>)}</div><RecommendationStatus id={item.id} /></article>)}</div> : <p className="mb-5 text-[var(--text-dim)]">No shared bottle queued yet.</p>}<GenerateRecommendations mode="joint" /></section>}
