    ...
    <div className="min-h-dvh md:grid md:grid-cols-[220px_1fr]">
      <aside className="hidden min-h-dvh border-r border-[var(--border)] p-5 md:flex md:flex-col" data-testid="desktop-rail">
        ...
        <div className="mt-auto space-y-2 border-t border-[var(--border)] pt-5">
          ...
          <button className="min-h-11 text-[var(--text-dim)]" type="button" onClick={logout}>LOG OUT</button>
        </div>
      </aside>
      <header className="flex h-14 items-center justify-between border-b border-[var(--border)] px-4 md:hidden">
        <span className="text-[var(--cyan)]">WINE_TRAINER</span>
        <Link href="/profiles" className={`bloom-${profile.color} no-underline`} style={{ color: `var(--${profile.color})` }}>{profile.name}</Link>
        <button className="min-h-11 text-xs text-[var(--text-dim)]" type="button" onClick={logout}>LOG OUT</button>
      </header>
