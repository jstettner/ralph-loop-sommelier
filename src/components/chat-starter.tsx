...
export function ChatStarter({ profiles, activeProfileId, models, defaultModel }: {
  ...
  return <section className="max-w-2xl border border-[var(--border)] bg-[var(--bg-raised)] p-6 md:p-10">
    <h1 className="bloom-cyan mb-2 text-base tracking-[0.18em] text-[var(--cyan)]">NEW TASTING SESSION</h1>
    <p className="mb-8 text-sm text-[var(--text-dim)]">Choose who is at the table. Participants stay fixed for this conversation.</p>
    <fieldset className="space-y-3"><legend className="mb-3 text-xs tracking-[0.12em]">WHO&apos;S TASTING?</legend>{profiles.map((profile) =>
      <label key={profile.id} className="flex min-h-14 cursor-pointer items-center gap-4 border border-[var(--border)] px-4 transition-colors hover:border-[var(--border-bright)]">
        <input className="accent-[var(--cyan)]" type="checkbox" checked={participants.includes(profile.id)} onChange={() => toggle(profile.id)} />
        <span className={`text-sm bloom-${profile.color}`} style={{ color: `var(--${profile.color})` }}>{profile.name}</span>
      </label>)}</fieldset>
    {profiles.length > 1 && <p className="mt-3 text-xs tracking-[0.08em] text-[var(--text-dim)]">+ TASTE TOGETHER: select another taster above</p>}
    <label className="mt-8 block"><span className="form-label">MODEL</span>
    ...
    <button className="terminal-button terminal-button--primary mt-8 w-full" type="button" onClick={start}>START CHAT</button>
  </section>;
}
